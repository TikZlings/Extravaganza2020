...
\usepackage{fontspec}
\newfontfamily\notoemoji{Noto Color Emoji}[Renderer=HarfBuzz,Opacity=0.25]

\usepackage{geometry}
\geometry{papersize={128mm,96mm},margin=0.5cm} %\text
...
% \put(0,-\paperheight){\textcolor{blue}{\rule{\paperwidth}{\paperheight}}}%
% \put(0.5\paperwidth,-0.5\paperheight){%
% \tikz[overlay]\shade[inner color=white,outer color=blue!50!white](0,0)circle[radius=0.7\textheight];%
% \raisebox{-0.35\height}{\makebox[0pt]{\fontsize{1.4\textheight}{6cm}\selectfont\notoemoji^^^^^^01f30d}}%
%}}

\foreach \x in  %{0,0.5,...,250}
   %{0,0.5,...,80}
   {0}
 {%
...
\coordinate (mycenter) at (0.5\textwidth,0.4\textheight);
 ...
 {
  \path[overlay] (mycenter) --+(\x+\y:1.85) pic { \y iduck };
 }
   \foreach \y in {0,30,...,330}%
 {
  \path[overlay] (mycenter) --+(\x+\y:3.5) pic { \y duck };
 }
 \path[overlay] (mycenter)--+(-0.5,0) pic {centerduck };
 \end{tikzpicture}\newpage
}
\end{document}
\makeatletter
...
\end{tikzpicture}
\begin{tikzpicture}
\bear\bearwear[shirt=white,t-shirt,body deco={\path (beartummy)--++(0.1,-0.1) pic[scale=0.04]{swisscross};}]
\begin{scope}
